...
function PropertyDetails() {
  ...
  function handlePost(e: React.FormEvent<HTMLFormElement>) {
    ...
    axios
      .post(`http://localhost:8080/api/host/${id}/properties`, newProperty)
      .then((res) => {
  ...
  function handleDelete(propertyId: number) {
    ...
    axios
      .delete(`http://localhost:8080/api/host/${id}/properties/${propertyId}`)
      .then((res) => {
        console.log("Property deleted:", res.data);
        setProperty(
          property.filter((property: any) => property.id !== propertyId)
        );
      })
      .catch((err) => {

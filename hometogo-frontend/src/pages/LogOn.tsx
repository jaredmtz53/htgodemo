...
const LoginPage = () => {
  ...
  const getUserProfile = async (accessToken: string) => {
    try {
      ...
      localStorage.setItem("googleUser", JSON.stringify(googleUser)); //Communicating to the backend the information needed for the user
      ...
      const appUser = response.data; // Data stored locally

      localStorage.setItem("user", JSON.stringify(appUser)); //When login successful redirect to our landing page
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6">
      ...
            
      <Button className="mt-4" variant="destructive" onClick={signOut}>
                Sign Out       
      </Button>
            
